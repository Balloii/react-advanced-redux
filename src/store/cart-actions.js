...
export const fetchCartData = () => {
    return async (dispatch) => {
        const fetchData = async () => {
            const response = await fetch("https://react-advanced-redux-60344-default-rtdb.asia-southeast1.firebasedatabase.app/cart.json");
...
export const sendCartData = (cart) => {
    return async (dispatch) => {
        ...
        const sendRequest = async () => {
            const response = await fetch("https://react-advanced-redux-60344-default-rtdb.asia-southeast1.firebasedatabase.app/cart.json", { method: "PUT", body: JSON.stringify({ items: cart.items, totalQuantity: cart.totalQuantity })})
