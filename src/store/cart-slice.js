    ...
    reducers: {
        ...
        removeItemFromCart(state, action) {
            ...
            if (existingItem.quantity === 1) {
                state.items = state.items.filter((item) => (item.id !== id)) //this will override old array with the new array.
            } else {
                existingItem.quantity--;
...
export const sendCartData = (cart) => {
    return async (dispatch) => {
        ...
        const sendRequest = async () => {
            const response = await fetch("https://react-advanced-redux-60344-default-rtdb.asia-southeast1.firebasedatabase.app/cart.json", { method: "PUT", body: JSON.stringify(cart)})
